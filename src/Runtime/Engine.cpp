...
void NextEngine::Start()
{
	...
	result = ma_engine_init(nullptr, audioEngine_.get());
...
glm::vec3 NextEngine::ProjectWorldToScreen(glm::vec3 locationWS)
{
	...
	transformed.x += 1.0f;
	transformed.x *= vkextent.width/2; // vulkan 特殊的坐标系分布
	transformed.y += 1.0f;
...
void NextEngine::GetScreenToWorldRay(glm::vec2 locationSS, glm::vec3& org, glm::vec3& dir)
{
	...
	glm::vec2 pixel = locationSS - glm::vec2(offset.x, offset.y); // pixcel 要减去vulkan自身的offset
...
void NextEngine::RayCastGPU(glm::vec3 rayOrigin, glm::vec3 rayDir, std::function<bool(Assets::RayCastResult rayResult)> callback)
{
...
Assets::UniformBufferObject NextEngine::GetUniformBufferObject(const VkOffset2D offset, const VkExtent2D extent)
{
	...
	// TODO 小细节，没有看懂这里是在做什么 理解一下projection matrix每个都代表什么含义，x轴翻转？
