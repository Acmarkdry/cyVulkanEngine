...
{
    if( std::filesystem::exists(std::string("/sdcard/Android/data/com.cynextrenderer/files/") + srcPath) )
    {
        return;
    }
    
    std::filesystem::create_directories(std::filesystem::path(std::string("/sdcard/Android/data/com.cynextrenderer/files/") + srcPath));
    ...
    while ((filename = AAssetDir_getNextFileName(assetDir)) != NULL){
        ...
        std::string targetPath = std::string("/sdcard/Android/data/com.cynextrenderer/files/") + srcPath + "/" + filename;
...
private:
    static void* logThreadFunction(void* arg) {
        ...
        while (true) {
            ...
            buffer[count] = '\0';
            __android_log_print(ANDROID_LOG_INFO, "cynext", "%s", buffer);
        }
