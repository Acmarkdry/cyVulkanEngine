...
using uo_map_tex_t = std::unordered_map<std::string, int32_t>;
...
namespace std
{
	...
	{
		size_t operator()(const Assets::Vertex& vertex) const noexcept
		{
			return
...
namespace Assets
{
	void ParseGltfNode(std::vector<Node>& out_nodes, CameraInitialSate& out_camera,
	                   std::vector<LightObject>& out_lights,
	                   mat4 parentTransform, tinygltf::Model& model, int node_idx, int modelIdx)
	{
		...
		{
			vec3 translation = node.translation.empty()
				                   ? vec3(0)
				                   : vec3(node.translation[0], node.translation[1], node.translation[2]);
			vec3 scaling = node.scale.empty() ? vec3(1) : vec3(node.scale[0], node.scale[1], node.scale[2]);
			quat quaternion = node.rotation.empty()
				                  ? quat(1, 0, 0, 0)
				                  : quat(
			...
			// 这里在计算四元数，四元数的标准形式是 w x y z

			quaternion = normalize(quaternion);
			mat4 t = translate(mat4(1), translation); // 平移矩阵
			mat4 r = toMat4(quaternion); // 旋转矩阵
			mat4 s = scale(mat4(1), scaling); // 缩放矩阵

			transform = parentTransform * (t * r * s); // 图形学里面要先进行 平移 旋转 缩放
		}
		else
		{
			mat4 localTs = make_mat4(node.matrix.data());
		...
		if (node.mesh != -1)
		{
			if (node.extras.Has("arealight"))
			{
				...
				auto local_p0 = vec4(-1, 0, -1, 1);
				auto local_p1 = vec4(-1, 0, 1, 1);
				auto local_p3 = vec4(1, 0, -1, 1);
				...
				* lightquad can represent by 4 points
				...
				auto dir = vec3(transform * vec4(0, 1, 0, 0));
				light.normal_area = vec4(normalize(dir), 0); // 法线和面积
				light.normal_area.w = length(cross(vec3(light.p1 - light.p0), vec3(light.p3 - light.p0))) / 2.0f;
		...
		else
		{
			if (node.camera >= 0)
			{
				vec4 camEye = transform * vec4(0, 0, 0, 1); // 相机的位置
				vec4 camFwd = transform * vec4(0, 0, -1, 0); // 相机的forward
				mat4 ModelView = lookAt(vec3(camEye), vec3(camEye) + vec3(camFwd.x, camFwd.y, camFwd.z), vec3(0, 1, 0));
				// 相机视图矩阵 
	...
	void Model::LoadGLTFScene(const std::string& filename, CameraInitialSate& cameraInit, std::vector<Node>& nodes,
	                          ...
	                          std::vector<Material>& materials, std::vector<LightObject>& lights)
	{
		...
		for (uint32_t i = 0; i < model.images.size(); ++i)
		{
			...
			uint32_t texIdx = GlobalTexturePool::LoadTexture(
				filepath.filename().string() + "_" + texname,
		...
		for (tinygltf::Material& mat : model.materials)
		{
			...
			vec3 emissiveColor = mat.emissiveFactor.empty()
				                     ? vec3(0)
				                     : vec3(mat.emissiveFactor[0], mat.emissiveFactor[1],
				                            mat.emissiveFactor[2]);
			vec3 diffuseColor = mat.pbrMetallicRoughness.baseColorFactor.empty()
				                    ? vec3(1)
				                    : vec3(mat.pbrMetallicRoughness.baseColorFactor[0],
				                           mat.pbrMetallicRoughness.baseColorFactor[1],
				                           mat.pbrMetallicRoughness.baseColorFactor[2]);

			m.Diffuse = vec4(sqrt(diffuseColor), 1.0);
		...
		for (tinygltf::Mesh& mesh : model.meshes)
		{
			...
			for (tinygltf::Primitive& primtive : mesh.primitives)
			{
				...
				{
					Vertex vertex;
					auto position = reinterpret_cast<float*>(&model.buffers[positionView.buffer].data[positionView.
						byteOffset + positionAccessor.byteOffset + i *
						positionStride]);
					...
					auto normal = reinterpret_cast<float*>(&model.buffers[normalView.buffer].data[normalView.byteOffset
						+ normalAccessor.byteOffset + i *
						normalStride]);
					vertex.Normal = vec3(
						normal[0],
						normal[1],
					...
					{
						auto texcoord = reinterpret_cast<float*>(&model.buffers[texcoordView.buffer].data[texcoordView.
							byteOffset + texcoordAccessor.byteOffset + i *
							texcoordStride]);
				...
				for (size_t i = 0; i < indexAccessor.count; ++i)
				{
					...
					{
						auto data = reinterpret_cast<uint16*>(&model.buffers[indexView.buffer].data[indexView.byteOffset
							+ indexAccessor.byteOffset + i * strideIndex]);
						indices.push_back(*data + vertext_offset);
					}
					else if (indexAccessor.componentType == TINYGLTF_COMPONENT_TYPE_UNSIGNED_INT)
					{
						auto data = reinterpret_cast<uint32*>(&model.buffers[indexView.buffer].data[indexView.byteOffset
							+ indexAccessor.byteOffset + i * strideIndex]);
						indices.push_back(*data + vertext_offset);
					}
					else if (indexAccessor.componentType == TINYGLTF_COMPONENT_TYPE_INT)
					{
						auto data = reinterpret_cast<int32*>(&model.buffers[indexView.buffer].data[indexView.byteOffset
							+ indexAccessor.byteOffset + i * strideIndex]);
						indices.push_back(*data + vertext_offset);
					}
					else
		...
		// default auto camera
		AutoFocusCamera(cameraInit, models);
		...
		{
			ParseGltfNode(nodes, cameraInit, lights, mat4(1), model, nodeIdx, modelIdx);
		}
	                        ...
	                        std::vector<LightObject>& lights, bool autoNode)
	{
		...
		for (const auto& _material : objReader.GetMaterials())
		{
			...
			{
				m = Material::DiffuseLight(
					vec3(material.emission[0], material.emission[1], material.emission[2]) * 100.f);
				// add to lights
			}
		...
		for (const auto& shape : objReader.GetShapes())
		{
			...
			if (autoNode)
			{
				nodes.push_back(Node::CreateNode(Utilities::NameHelper::RandomName(6), mat4(1),
				                                 static_cast<int>(models.size()) - 1, false));
			}
		}
	...
	Model Model::CreateBox(const vec3& p0, const vec3& p1, int materialIdx)
	{
		...
		std::vector<uint32_t> materialids = {static_cast<uint32_t>(materialIdx)};
	...
	Model Model::CreateSphere(const vec3& center, float radius, int materialIdx, const bool isProcedural)
	{
		...
		std::vector<uint32_t> materialIdxs = {static_cast<uint32_t>(materialIdx)};
	...
	int Model::CreateLightQuad(const vec3& p0, const vec3& p1, const vec3& p2, const vec3& p3,
	                           const vec3& dir, const vec3& lightColor,
	                           std::vector<Model>& models,
	                           std::vector<Material>& materials,
	                           std::vector<LightObject>& lights)
	{
		...
		materialIdx_(std::move(materials))
	{
		...
		local_aabb_min = vec3(999999, 999999, 999999);
		local_aabb_max = vec3(-999999, -999999, -999999);
	...
	Node Node::CreateNode(std::string name, mat4 transform, int id, bool procedural)
	{
		return Node(name, transform, id, procedural);
	}

	Node::Node(std::string name, mat4 transform, int id, bool procedural): name_(name), transform_(transform),
	                                                                       modelId_(id),
	                                                                       procedural_(procedural)
	{
	}
