...
namespace Assets
{
	...
	device_(device),commandPool_(command_pool),mainThreadCommandPool_(command_pool_mt)
	{
		...
		// create
		VkDescriptorPoolSize pool_size_bindless[] = {
		...
		poolInfo.pPoolSizes = pool_size_bindless;
		poolInfo.maxSets = k_max_bindless_resources*1;

		Vulkan::Check(vkCreateDescriptorPool(device_.Handle(),&poolInfo,nullptr,&descriptorPool_),
			"create globals descriptor pool");
		...
		VkDescriptorSetLayoutBindingFlagsCreateInfoEXT extend_info{VK_STRUCTURE_TYPE_DESCRIPTOR_SET_LAYOUT_BINDING_FLAGS_CREATE_INFO_EXT,nullptr};
		extend_info.bindingCount = 1;
		extend_info.pBindingFlags = &bindless_flags;
	...
	uint32_t GlobalTexturePool::RequestNewTextureFileAsync(const std::string& filename, bool hdr)
	{
		...
		{
			return textureNameMap_.at(filename);
		}
