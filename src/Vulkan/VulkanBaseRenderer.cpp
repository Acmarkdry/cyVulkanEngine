...
namespace Vulkan
{
    ...
    void VulkanBaseRenderer::SetPhysicalDeviceImpl(VkPhysicalDevice physicalDevice, std::vector<const char*>& requiredExtensions, VkPhysicalDeviceFeatures& deviceFeatures, void* nextDeviceFeatures)
    {
        deviceFeatures.fillModeNonSolid = true; // 非实心模式，也就是
        deviceFeatures.samplerAnisotropy = true; // 各向异性过滤支持
        ...
        // vk khr shader clock 扩展 提供准确的着色器时钟指令
...
#endif
        indexingFeatures.runtimeDescriptorArray = true; // runtime阶段动态描述符数组
        indexingFeatures.shaderSampledImageArrayNonUniformIndexing = true; // shader采样阶段非统一纹理索引
        indexingFeatures.descriptorBindingPartiallyBound = true; // 部分绑定
        indexingFeatures.descriptorBindingSampledImageUpdateAfterBind = true; // 绑定后更新纹理
        indexingFeatures.descriptorBindingVariableDescriptorCount = true; // 可变数量描述符


        VkPhysicalDeviceBufferDeviceAddressFeatures bufferDeviceAddressFeatures = {};
        bufferDeviceAddressFeatures.sType = VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_BUFFER_DEVICE_ADDRESS_FEATURES;
        bufferDeviceAddressFeatures.pNext = &indexingFeatures;
        bufferDeviceAddressFeatures.bufferDeviceAddress = true; // 允许在着色器中直接通过地址访问缓冲区
        ...
        hostQueryResetFeatures.hostQueryReset = true; // 允许从cpu重置查询池
