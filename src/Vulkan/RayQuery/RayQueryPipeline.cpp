...
namespace Vulkan::RayTracing
{
        ...
        const std::vector<Assets::UniformBuffer>& uniformBuffers, const Assets::Scene& scene):swapChain_(swapChain)
    {
        ...
        const std::vector<DescriptorBinding> descriptorBindings =
        {
            ...
            {0, 1, VK_DESCRIPTOR_TYPE_ACCELERATION_STRUCTURE_KHR, VK_SHADER_STAGE_COMPUTE_BIT}, // 加速结构
            // Light buffer
            {1, 1, VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, VK_SHADER_STAGE_COMPUTE_BIT}, // 光照缓冲区
            // Camera information & co
            {3, 1, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, VK_SHADER_STAGE_COMPUTE_BIT}, // uniform 缓冲区

            // Vertex buffer, Index buffer, Material buffer, Offset buffer
            {4, 1, VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, VK_SHADER_STAGE_COMPUTE_BIT}, // vertex数据缓冲区
            ...
            {10, 1, VK_DESCRIPTOR_TYPE_STORAGE_IMAGE, VK_SHADER_STAGE_COMPUTE_BIT}, // storage image
        ...
        for (uint32_t i = 0; i != swapChain.Images().size(); ++i)
        {
            ...
            indexBufferInfo.range = VK_WHOLE_SIZE; // 表示offset到缓冲区末尾的所有字节
