...
Options::Options(const int argc, const char* argv[])
{
	...
	renderer.add_options()
		("renderer", value<uint32_t>(&RendererType)->default_value(5), "Renderer Type (0 = RayTraced, 1 = ModernDeferred, 2 = LegacyDeferred, 3 = RayQuery, 4 = HybridDeferred, 5 = VulkanBase).")
		("samples", value<uint32_t>(&Samples)->default_value(4), "The number of ray samples per pixel.")
		("bounces", value<uint32_t>(&Bounces)->default_value(10), "The general limit number of bounces per ray.")
