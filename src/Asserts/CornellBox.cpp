...
namespace Assets
{
	namespace
	{
		...
		{
			indices.emplace_back(offset + i0);
			indices.emplace_back(offset + i1);
			indices.emplace_back(offset + i2);
		}
		
	}

	void CornellBox::Create(float scale, std::vector<Vertex>& vertices, std::vector<uint32_t>& indices, std::vector<Material>& materials, std::vector<LightObject>& lights)
	{
		// todo 看不懂这里是在干什么
