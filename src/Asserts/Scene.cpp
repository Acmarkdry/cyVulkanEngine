...
namespace Assets
{
	...
	Scene::Scene(Vulkan::CommandPool& commandPool, std::vector<Node>& nodes, std::vector<Model>& models, std::vector<Material>& materials, std::vector<LightObject>& lights, bool supportRayTracing):
	materials_(materials),models_(models),nodes_(nodes)
	{
		// todo 这里的代码要好好研究一下
		...
		for(const auto& model:models_)
		{
			...
			vertices.insert(vertices.end(),model.Vertices().begin(),model.Vertices().end()); // 批量插入 在vertices.end() - 1插入后面的一大坨数据
		...
		for(int i = 0;i < models_.size();i++)
		{
			...
			for (const auto& node : nodes_)
			{
				if(node.GetModel() == i)
				{
					nodeProxies.push_back(NodeProxy{ node.WorldTransform() });
		...
		Vulkan::BufferUtil::CreateDeviceBuffer(commandPool,"Nodes",flags,nodeProxies,nodeMatrixBuffer_,nodeMatrixBufferMemory_);
